...
Our goal is to understand the implications of passive symmetries for machine learning: Which passive symmetries play a role (e.g., permutation symmetry in graph neural networks)? What are \emph{dos and don'ts} in machine learning practice if passive symmetries are to be respected?
...
This paper is purely conceptual;
it is intended to help with translation among physics, mathematics, and machine-learning languages.
We believe that consideration of passive symmetries can help machine learning make the transition that took place for modern physics in the first half of the twentieth century.
\end{abstract}
...
Recent ML work in this direction is beautifully reported in \cite{weiler} and \cite{bronstein2021geometric}.
...
This shows that a function can sometimes be inferred from units covariance only, i.e., from a purely passive symmetry, combined with the \emph{empirical} knowledge that the solution is \emph{independent} of all other observables.
...
This is a form of qualitative \emph{transfer} that we expect will also become relevant for model transfer in ML \cite{RojSchTurPet18}.
