...
While this paper is purely conceptual, we believe that it can have an impact on helping machine learning make the transition that took place for modern physics in the first half of the Twentieth century.
\end{abstract}
...
When features and labels are represented in a mathematical form that involves investigator choices, methods of ML (or any relevant function) ought to be written in a form that is exactly equivariant to changes in those investigator choices.
...
physical entities.
\end{quote}
This principle leads to the important physical symmetries of coordinate freedom and gauge symmetry; a small generalization would include what we will refer to as units covariance.
...
We argue---in this purely conceptual contribution---that analogs of these symmetries could have impact in ML, and increase the scope of group-equivariant methods in ML.
...
An example familiar to physicists is what we call \emph{units covariance}, which is the requirement that any correct description of the world have inputs and outputs with the correct units: \bernhard{NEW: imagine an equation connecting physical observables that numerically holds true. If the left hand side and the right hand side of the equation were to contain different dimensions, then we could disrupt the numerical correctness of the equation by changing the units using which we measure some of the observables (e.g., from kg to pounds).}
...
While some of the below terms will only be described in detail further below, we start with a glossary to facilitate reading the paper.
\bernhard{could quote Halmos here: "... the reader should not feel discouraged if he finds he does not have the prerequisities for reading the prerequisites" }
...
\item[covariance:]
When a physical law is written in a way that is equivariant with respect to all (or some) passive symmetries, then the law is said to be \emph{covariant}.
...
Equivariances define symmetries in the space of maps.  \hogg{ADD COMMENT ABOUT ACTIVE ONLY HERE??}
\item[gauge freedom:]
...
\item[group action:] We consider a group $G$, a space $X$, and a map $\rho: G \to \operatorname{Sym}(X)$, where $\operatorname{Sym}(X)$ denotes the bijective maps from $X$ to itself. Using a slight use of notation, sometimes this map is expressed as $\rho:G\times X \to X$. We say $\rho$ is an action of $G$ on $X$ if it satisfies the following properties (1) Identity: $\rho(e,x)=x$ for all $x\in X$. (2) Compatibility: $\rho(a, \rho(b, x))=\rho (a\cdot b, x)$.
...
In contrast, the fact that the protein folds in a way that doesn't depend on the coordinate system {\em chosen to describe it} is absolutely and always true; it is not experimentally established; it is a passive symmetry.
...
depending on the number of unsummed indices), and the function will be precisely 
%equivariant\bernhard{in our new terminology: 
covariant to rotations and reflections of the coordinate system.
...
\hogg{We need a paragraph about the relationship between active and passive symmetries, and how they can be converted.}
...
Einstein, for example, found the equations of general relativity by looking at every expression to some degree consistent with the passive symmetry enforced by the Ricci calculus (he called this family of functions \emph{covariant}) until he found the one that reduced to Newtonian gravity in the weak-field limit \citep{gr}.
Following this lead, we will refer to the passive symmetries as ``covariances'' in what follows.
Even before general relativity, special relativity \citep{sr} can be seen as making the decision to move an active symmetry (the observation that the speed of light is the same for all observers) into a passive symmetry, true by construction for all valid physical laws.
This passive symmetry is now called Lorentz invariance.
We stress that these insights were profound \citep{EARMAN1978251}; they revolutionized physics; not too long before Einstein, such considerations would have seemed highly unusual. 
Similarly, passive symmetries do not feature in today's ML practice---if the development of physics is any indication, their potential could be significant.
...
Assume that only $m, g, h$ come in to the solution;\footnote{We will return to this seemingly innocuous point below. We note that it is an {\em empirical} statement, which will turn out to have rather significant implications.} in particular, assume that the height $h$ and the velocity $v$ are both small enough that air resistance, say, can be ignored.
...
This shows that a function can sometimes be inferred from units covariance only, i.e., from a purely passive symmetry, combined with the {\em empirical} knowledge that the solution is {\em independent} of all other observables.
%\bernhard{I now believe this is {\em not} completely true: crucially, it needs empirical input too: the fact that the solution is {\em independent} of all other observables.}
...
Consider $\mathcal{X}$ to be space of all possible physical states of a specific system (for instance $x\in \mathcal X$ could be the positions, velocities, masses, spins, and charges of a set of particles at all times). 
...
That is, any element $z\in \mathcal{H}$ will be a list of real values of vector and tensor components, and scalars.\bernhard{at all times? (we decided to delete this, I think}
...
\hogg{Related to the above: Each layer must be connected to the previous layer by enough edges that the weight multiplication can be seen as a dot product or contraction. That might have implications for dropouts or other kinds of sparse structures.}
...
Data normalization, batch normalization, and layer normalization are all generally brutal and violate model equivariances and covariances.
These provide rich examples, one of which we expand further in \secref{sec:norm}.
...
It neither makes sense to add or average (nor add nor average in the square) features with different units, nor does it make sense to apply very different scales to different features with the same units and qualitatively similar measurement properties.
...
At application time, the 3 components of any 3-vector must be scaled identically, not independently, otherwise the vector is (in general) arbitrarily rotated.
In this 3-vector case, he shift $\mu$ cannot be a single number but instead, for the 3-vectors, the shift must itself be a 3-vector.
That's natural in many but not all normaliztion methods in use at present.
...
Once again, at application time, the tensor components cannot be independently scaled with 9 different scales; instead one scale must be applied consistently to all 9 components.
...
%\soledad{DO WE NEED TO MENTION AND CITE Equivariant Representation learning (https://arxiv.org/abs/2012.02771)? Here or somewhere above in Related Work?}
