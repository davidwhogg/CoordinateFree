...
We are inspired in part by the success of message passing in graph neural networks (\hogg{CITE});
message-passing algorithms exactly obey the passive symmetry corresponding to node relabelings.
We are also inspired in part by the success of convolutional neural networks in image-analysis contexts (\hogg{CITE}) in which translation symmetry is not an active symmetry, but where the particular pixel locations of particular features are unimportant to the recognition task at hand.
