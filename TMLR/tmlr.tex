...
%% US: New experiment: covariance in data normalization

%% US: We will publish all the notebooks in a single github repository, so that they are very easy to run.

%% HOGG SAYS THIS IS NOW DONE: To the list of contributions we give in the Introduction, we will add a specific bullet point about the circumstances in which an additional learned feature converts a covariant problem into an actively equivariant problem.

%% US: We will add a description of the experimental setup in the appendix.

%% REFEREE: In the abstract, the paper arguably translates between different languages of physics, mathematics, and machine learning. Although I see such traces in the glossary, these vocabularies are not fundamental in the later discussion.
%% FIXED
%% HOGG SAYS THIS IS NOW DONE: This point is fair; we will reduce the emphasis on this point in the abstract, move the glossary to the appendix, and take care to make translations in the text where they are useful.

%% US: We will add a description of the different methods in the literature and experimental results in an appendix.
%TODO: SOLEDAD

%% REFEREE: The paper feels unnecessarily long, and (at least in the first 6 sections) somewhat redundant/repetative. I think that this paper could fit within the standard 12 page limits of a TMLR submission without needing to cut much content. 
%% NEARLY FIXED

%Move the glossary to the appendix (it is very helpful, but disrupts the reading flow) 
%FIXED

% REFEREE: Sections 3-5 seem to repeat many of the same ideas but with slight variation. It is possible that including the formal definitions of Section 5 before introducing the content in Section 3 could reduce the need to re-introduce passive and active symmetries multiple times.
% WON'T FIX -- I THINK WE ARE CLOSE

% HOGG SAYS THIS IS NOW NEARLY DONE: We will take all of these suggestions; these are all useful and reasonable. We agree that the paper can fit in the 12 pages with some reasonable changes.

%We will write two new appendices, one on the experiment that’s there now, and one on the new experiment (described above).

%HOGG: write the appendix on the black body experiment

%Page 6 towards the bottom "sometimes Einstein summation notation, Einstein 1916" <- should this be "sometimes called Einstein summation notation"?
%FIXED

%Page 8 "The two approaches are theoretically eqiuvalent" <- should be "three" approaches
%FIXED

% HOGG say, for the pendulum problem: In the same way that the g vector is critical for making the problem rotation invariant, the pivot position is critical for making the problem translation invariant, and the pivot velocity plus the two masses are critical for making the problem boost-invariant. All this flows from Weichi's normalization experiments.

\documentclass[]{article} % [preprint] for arXiv, [] for submission
\usepackage{tmlr}
...
If the model doesn't have access to all relevant vectors as inputs then the predictions will not be apparently $O(3)$ equivariant.
...
\section{Units covariance regression: the black body experiment}
\label{app.dimensionless}
